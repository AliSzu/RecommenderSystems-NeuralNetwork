...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        :rtype: pd.DataFrame
        """
        df["weekend_stay"] = np.busday_count(df['date_from'].values.astype('datetime64[D]'),
                                             df['date_to'].values.astype('datetime64[D]'),
                                             weekmask="Fri Sat")
        df["weekend_stay"] = df["weekend_stay"] > 0
    ...
    def aggregate_group_reservations(self, df):
        ...
        group_reservations_mode = group_reservations.groupby('group_id')[self.mode_columns].agg(
            lambda x: x.value_counts().index[0])
        group_reservations_first = group_reservations.groupby('group_id')[self.first_columns].first()

        group_reservations = pd.concat(
            [group_reservations_sum, group_reservations_mean, group_reservations_mode, group_reservations_first],
            axis=1)
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        mean_night_price = df.groupby('room_group_id')['night_price'].mean().reset_index()
        mean_night_price.columns = ['room_group_id', 'night_price_mean']
        df = df.merge(mean_night_price, how='left', on='room_group_id')
        df['room_segment'] = df['night_price_mean'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        df.drop(columns=["night_price_mean"])
        return df
